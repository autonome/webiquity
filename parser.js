...
var NLParser2 = {
  ...
  // Namespace object
  parserFactories: {},
  makeParserForLanguage: function(languageCode, verbList, suggestionMemory) {
    if (!(languageCode in this.registry)) {
      try {
        loadParserMaker(languageCode)
      }
      catch (e) {
  ...
  loadParserMaker: function loadParserMaker(code, makeParser) {
    this.parserFactories[code] = makeParser;
  }
}
...
Parser.prototype = {
  // References to suggestionMemory module; makeParserForLanguage()
  ...
  // Nouns are cached in this cache in the {{{Parser#detectNounType}}} method
  ...
  // Build up all the metadata used to match user input
    ...
    for each (let verb in verbs) {
      for each (let arg in verb.arguments) {
        ...
        if (!nouns[id]) {
          ...
          let {input} = arg
          if (input) {
            if (!nounIdsForGivenInputs[input]) {
              nounIdsForGivenInputs[input] = {}
              nounIdsForGivenInputs[input][id] = true
            }
            return nounIdsForGivenInputs[input]
          }
          */
        }
      }
  ...
  verbFinder: function(input, selection) {
    ...
    function boostVerbScoreWithFrequency(score, inputPart, verbId) {
      ...
      var reinforcement = self._suggestionMemory.getScore(inputPart, verbId) + 1;
    ...
    if (this._patternCache.verbInitialTest) {
  ...
  hasDelimiter: function(delimiter, verbId) {
    return this._patternCache.delimiters[verbId] ? this._patternCache.delimiters[verbId].test(delimiter) : false
  },
  ...
  argFinder: function argFinder(argString, verb, input, thisQuery) {
    ...
    var possibleDelimiterCombinations = Utils.powerSet(possibleDelimiterIndices);
  ...
  suggestVerb: function suggestVerb(parse, inputMatchesSomeVerb) {
    ...
    for each (let verb in verbs) {
      ...
      let parser = this;
      for (let role in parse.args){
        if (!verb.arguments || !verb.arguments.some(function (arg) {
...
Parse.prototype = {
  ...
  get previewUrl() {
    var argId = this._verb.arguments[0].id,
