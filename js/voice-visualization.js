...
var visualSetting = 'sinewave', // or 'frequencybars'
    canvasNode = document.querySelector('.visualizer');
...
function onMediaStreamHandler(stream) {
  ...
  visualize(canvasNode, analyser, visualSetting);
}


/*

Visualize an audio node as either a sinewave or frequency bars.

*/
function visualize(canvas, analyserNode, visualSetting) {
  ...
  function sinewaveViz(canvas, analyserNode) {
...
// Get a specified number of random selections from a set.
...
// Generate colors
// returns rgb() string (TODO make general purpose)
function getColors() {
  var scheme = new ColorScheme;
  scheme.from_hue(21).scheme('triade').variation('hard');
  var palette = scheme.colors();
